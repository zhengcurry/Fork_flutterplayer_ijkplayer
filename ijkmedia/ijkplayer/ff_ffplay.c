...
static int convert_image(FFPlayer *ffp, AVFrame *src_frame,
    ...
    if (is->viddec.avctx) {
        av_reduce(&display_aspect_ratio.num, &display_aspect_ratio.den,
                  ...
                  (int64_t)
        is->viddec.avctx->sample_aspect_ratio.num,
                is->viddec.avctx->height *
                (int64_t)
        is->viddec.avctx->sample_aspect_ratio.den,
                1024 * 1024);
    ...
    buffer = (uint8_t * )
    av_malloc(bytes * sizeof(uint8_t));
    if (!buffer) {
        ret = -1;
...
//            av_log(NULL, AV_LOG_ERROR, "decoder_decode_frame");
...
//      delay = compute_target_delay(ffp, last_duration, is);
            delay = 0.005;
//        av_log(NULL, AV_LOG_ERROR, "curry delay---------------%f",delay);
        ...
        if (!isnan(pts)) {
            ...
            if ((pts * 1000 * 1000 < is->seek_pos) || deviation > MAX_DEVIATION) {
                ...
                if (is->drop_vframe_count == 0) {
                    ...
                    av_log(NULL, AV_LOG_INFO,
                           "video accurate_seek start, is->seek_pos=%"
                    PRId64
                    ", pts=%lf, is->accurate_seek_time = %"
                    PRId64
                    "\n",
                ...
                if (!isnan(frame_pts)) {
                    ...
                    if ((audio_clock * 1000 * 1000 < is->seek_pos) ||
                        ...
                        if (is->drop_aframe_count == 0) {
                            ...
                            av_log(
                                    NULL, AV_LOG_INFO,
                                    "audio accurate_seek start, is->seek_pos=%"
                            PRId64
                            ", audio_clock=%lf, is->accurate_seek_start_time = %"
                            PRId64
                            "\n",
        ...
        if (ffp->get_frame_mode) {
            ...
            if (pts >= dst_pts) {
                while (retry_convert_image <= MAX_RETRY_CONVERT_IMAGE) {
                    ret = convert_image(ffp, frame, (int64_t)
                    pts, frame->width,
                            frame->height);
                    if (!ret) {
                        convert_frame_count++;
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len) {
    ...
    while (len > 0) {
        if (is->audio_buf_index >= is->audio_buf_size) {
            ...
            if (audio_size < 0) {
                ...
                if (is->show_mode != SHOW_MODE_VIDEO)
                    update_sample_display(is, (int16_t * )
                is->audio_buf, audio_size);
        ...
        if (!is->muted && is->audio_buf && is->audio_volume == SDL_MIX_MAXVOLUME)
            memcpy(stream, (uint8_t * )
        is->audio_buf + is->audio_buf_index, len1);
        else {
            memset(stream, 0, len1);
            if (!is->muted && is->audio_buf)
                SDL_MixAudio(stream, (uint8_t * )
...
//    av_log(NULL, AV_LOG_ERROR, "curry---------------");
#endif
    if (!avctx)
        return AVERROR(ENOMEM);
...
static int read_thread(void *arg) {
    ...
    is->max_frame_duration = 0.03;
    av_log(ffp, AV_LOG_ERROR, "max_frame_duration: %.3f\n",
           is->max_frame_duration);
     ...
     * discard */
        ...
        (pkt_ts -
         (stream_start_time != AV_NOPTS_VALUE ? stream_start_time :
        0)) *
...
int ffp_get_video_rotate_degrees(FFPlayer *ffp) {
    ...
    int theta =
    abs((int) ((int64_t)
    round(fabs(get_rotation(is->video_st))) % 360));
    switch (theta) {
        case 0:
        case 90:
        case 180:
    ...
    if (ffp->is_record) {
        ...
        if (packet != NULL && packet->size > 0 && packet->data != NULL) {
            if (0 == av_packet_ref(pkt, packet)) {
                av_log(ffp, AV_LOG_INFO, "ffp->start_pts:%"
                PRId64
                "", ffp->start_pts);
                av_log(ffp, AV_LOG_INFO, "ffp->start_dts:%"
                PRId64
                "", ffp->start_dts);
                ...
                av_log(ffp, AV_LOG_INFO, "inner pkt->pts:%"
                PRId64
                "\n", pkt->pts);
                av_log(ffp, AV_LOG_INFO, "inner pkt->dts:%"
                PRId64
                "\n", pkt->dts);
                av_log(ffp, AV_LOG_INFO, "inner pkt->duration:%"
                PRId64
                "\n",
                        pkt->duration);
...
void ffp_get_current_frame_l(FFPlayer *ffp, uint8_t *frame_buf) {
    ...
    ALOGD("=============>3 %d", pixels);
    for (i = 0; i < height; i++) {
        ALOGD("=============>4 frame_buf  %d ", frame_buf);
        ALOGD("=============>4 i  %d ", i);
        ALOGD("=============>4 pixels %d", pixels);
        ALOGD("=============>4 linesize %d", linesize);
        ALOGD("=============>4 src %d", src);
        ALOGD("=============>5 %s %s", frame_buf + i * pixels, src + i * linesize);
...
int ffp_record_file(FFPlayer *ffp, AVPacket *packet)
{
    av_log(ffp, AV_LOG_ERROR, "ffp_record_file");
    assert(ffp);
    VideoState *is = ffp->is;
    int ret = 0;
    AVStream *in_stream;
    ...
    if (ffp->is_record) {
        ...
        if (packet == 0) {
            ffp->record_error = 1;
            av_log(ffp, AV_LOG_ERROR, "packet == 0");
            return -1;
        }
        //解决缓存为null导致崩溃的问题
        if (packet->buf == NULL) {
            ffp->record_error = 1;
            av_log(ffp, AV_LOG_ERROR, "packet->buf == NULL");
            return -1;
        }
        av_log(ffp, AV_LOG_ERROR, "packet buf size -> %lld", packet->buf->size);
        if (packet->buf->size <= 0) {
            ffp->record_error = 1;
            av_log(ffp, AV_LOG_ERROR, "packet->buf <=0");
            return -1;
        }
        ...
        if (packet->buf->size > 2 && 0 == av_packet_ref(pkt, packet)) {
            av_log(ffp, AV_LOG_ERROR, "packet buf -> %lld", packet->buf);
            av_log(ffp, AV_LOG_ERROR, "packet buf size -> %lld", packet->buf->size);
            pthread_mutex_lock(&ffp->record_mutex);
            //录制的第一帧，时间从0开始
            if (!ffp->is_first) {
                ...
                if (pkt->stream_index == AVMEDIA_TYPE_VIDEO && (pkt->flags & AV_PKT_FLAG_KEY)) {
                    ...
                    pkt->pts = 0;
                    pkt->dts = 0;
                    ffp->record_starting = 1;
                    av_log(ffp, AV_LOG_ERROR, "录制：第一针");
                }else{
                    av_packet_unref(pkt);
                    pthread_mutex_unlock(&ffp->record_mutex);
                    ffp->record_starting = 0;
                    av_log(ffp, AV_LOG_ERROR, "录制：还没到关键帧");
                    return ret;
                }
            } else {
...
/*int ffp_get_current_frame(FFPlayer* ffp,const char* out_file) {
    ALOGD("=============> ffp_get_current_frame");
	if (!ffp->is_screenshot){
	    ALOGD("=============> ffp_get_current_frame is_screenshot == 1");
		ffp->is_screenshot = 1;
