...
static jint
IjkMediaPlayer_startingRecord(JNIEnv *env, jobject thiz)
{
    jint retval = 0;
    IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
    JNI_CHECK_GOTO(mp, env, NULL, "mpjni: startingRecord: null mp", LABEL_RETURN);
    retval = ijkmp_record_starting(mp);
    LABEL_RETURN:
    ijkmp_dec_ref_p(&mp);
    return retval;
}
...
static JNINativeMethod g_methods[] = {
    ...
    { "startingRecord",         "()I",         (void *) IjkMediaPlayer_startingRecord},
};
